...
###############################lablel###
    labpro=Label(proo,image=photo1,bg='#ffffff').place(x=50,y=50)
###################################frame
    fr=Frame(proo,width=400,height=400,bg='#ffffff').place(x=450,y=50)
################################label
...
#############################################
    def signup():
        ...
        if password == conform :
            ...
            messagebox.showinfo('sign up','sucessfully sing up')
            exit()
...
################################# button 
    ...
    loginbutton=Button(proo,text='login',bd=0,bg='#ffffff',cursor="hand2",width=15,fg='#2F98F1',font='bold 10',command=login)
...
####____________________________ login ___________________#####

################################################################
def loginadmin():
    pro =Tk()
    pro.geometry('900x600+100+5')
    pro.configure(bg='#ffffff')
    pro.title('login')
    pro.resizable(False,False)


    photo1=PhotoImage(file='G:\programming\student\login.png')
###############################lablel###
    labpro=Label(pro,image=photo1,bg='#ffffff').place(x=50,y=50)
###################################frame
    fr=Frame(pro,width=400,height=400,bg='#ffffff').place(x=450,y=50)
################################label
...
##########################################username
    ...
    user=Entry(fr,width=20,fg='#000000',bd=0,bg='#ffffff',font='bold 15')
    user.place(x=550,y=100)
    user.insert(0,'username')
    Frame(fr,width=225,height=2,bg='#000000').place(x=550,y=125)
    user.bind('<FocusIn>',enter)
    user.bind('<FocusOut>',leave)
############################################# password
    def enter(e):
    ...
    code=Entry(fr,width=20,fg='#000000',bd=0,bg='#ffffff',font='bold 15')
    code.place(x=550,y=200)
    code.insert(0,'password')
    Frame(fr,width=225,height=2,bg='#000000').place(x=550,y=225)
    code.bind('<FocusIn>',enter)
    code.bind('<FocusOut>',leave)
####################################################
    signin=Button(fr,width=40,pady=7,text='sign in',bg='#2F98F1',fg='#ffffff',bd=0,command=signin).place(x=550,y=300,)
###################################
    lb1=Label(fr,text="Don't have an account ?",fg='#000000',bg='#ffffff',font='bold 9')
    lb1.place(x=550 ,y= 230)
    signup=Button(fr,text='sign up now',bd=0,bg='#ffffff',cursor="hand2",width=10,fg='#2F98F1',command=signup)
    signup.place(x=690 ,y= 230)
...
#############################################-------showimage
    def showimage():
        ...
        messagebox.showinfo('upload',' Uploaded Successfully image')
 ##################################################################### clear
    def Restart ():
...
# ###################################save
    def save():
        ...
        if nameget=='' or ageget=='' or phoneget == '' or mailget == '' or formget == '' or numberget == '' or genderget == '' or paymentget == '' or dateget =='' :
            ...
            messagebox.showinfo('info','sucessfully')    
...
########################################################################################frame 1
    ...
    photo=PhotoImage(file='G:\programming\student\search.png')

    entphoto=Button(fr1,image=photo,width=90,bd=0,height=60).place(x=200,y=40)
...
###########################################################################3
    photo2=PhotoImage(file='G:\programming\student\im.png')
